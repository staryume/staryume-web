...
| Summer questionnaire | `survey.html` / `survey-dashboard.html`: `noindex` + robots Disallow; submit uniqueness is email+event; dashboard passcode is server-side |
...
1b. Review **STARYUME Survey · Summer 2026** for spam rows; do not share dashboard screenshots that include emails.  
...
- **Summer questionnaire dashboard** (`survey-dashboard.html`): staff passcode is checked **server-side** on every `/api/survey` staff action (Apps Script `SURVEY_PASSCODE` / Script Property). The page URL is not secret. Confirmation serials + emails are PII — do not screenshot the dashboard into public Discord. See `docs/survey-apps-script.md`.  
